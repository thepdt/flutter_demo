...
class CheckoutService extends BlocService<OrderingModel> {
  ...
  Future<OrderingModel> get(int id) async {
    // TODO: implement get
    var rs = await HttpHelper.get('$ORDERING_ENDPOINT/$id',
  ...
  Future<List<OrderingModel>> getAll({int from = 0, int limit}) async {
    ...
    var rs = await HttpHelper.get(ORDERING_ENDPOINT,
        bearerToken: currentLogin.token);
    if (rs.statusCode == 200) {
...
//  }
  ...
  Future<OrderingModel> checkOut(OrderingModel orderingModel,
      {int status, int deliveryId}) async {
