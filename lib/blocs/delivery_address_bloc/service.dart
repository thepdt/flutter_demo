...
class DeliveryAddressService extends BlocService<DeliveryAddressModel> {
  ...
  Future<DeliveryAddressModel> get(int id) async {
    // TODO: implement get
    var rs = await HttpHelper.get('$DELIVERY_ADDRESS_ENDPOINT/$id',
  ...
  Future<List<DeliveryAddressModel>> getAll({int from = 0, int limit}) async {
    ...
    var rs = await HttpHelper.get(DELIVERY_ADDRESS_ENDPOINT,
        bearerToken: currentLogin.token);
    if (rs.statusCode == 200) {
  ...
  Future<DeliveryAddressModel> addDeliveryAddress(
      DeliveryAddressModel deliveryAddressModel) async {
